...
from grain_in.models import AcquisitionVarieties, GBCornInformations, Packaging, VehicleType,VehicleProvince
from login.models import UserPermissions,User_local
...
def vehicle_default():
    ...
    connection.close()
    for ll in VLPLATE:
        defaultins = VehicleProvince.objects.create(code_id=str(
            ll[0]), province_id=ll[1], province_name=ll[2], province_code=ll[3], city_id=ll[4], city_name=ll[5], city_code=ll[6])
        defaultins.save()
...
def acquisition_defaule():
    ...
    for ll in DEFAULT:
        ...
        c = "".join(b)
        defaultins = AcquisitionVarieties.objects.create(
            AV_name=ll[1], find_code=ll[0], py_abbreviation=c, acquisitioncaps=ll[2])
        defaultins.save()
...
def GBforCorn():
    DEFAULT = [
        ['一等', 1000, 720, 4.0, 1, 1, 1, 1, 2.0, 1,
            1, 1, 1.0, 1, 1, 1, '正常', '正常', 14.0, 1],
        ['二等', 720, 690, 6.0, 1, 1, 1, 1, 2.0, 1, 1,
            1, 1.0, 1, 1, 1, '正常', '正常', 14.0, 1],
        ['三等', 690, 660, 8.0, 1, 1, 1, 1, 2.0, 1, 1,
            1, 1.0, 1, 1, 1, '正常', '正常', 14.0, 1],
        ['四等', 660, 630, 10.0, 1, 1, 1, 1, 2.0, 1,
            1, 1, 1.0, 1, 1, 1, '正常', '正常', 14.0, 1],
        ['五等', 630, 600, 15.0, 1, 1, 1, 1, 2.0, 1,
            1, 1, 1.0, 1, 1, 1, '正常', '正常', 14.0, 1],
        ['等外', 600, 0, 0, 1, 1, 1, 1, 2.0, 1, 1,
            1, 1.0, 1, 1, 1, '正常', '正常', 14.0, 1],
    ]
    ...
    for ll in DEFAULT:
        conn = GBCornInformations.objects.create(acquisitionVarietiesid=acquisition_name, GB_filename=file_name, GB_file_contents=file_contents, purchase_price=0, purchase_name=ll[0], test_weight_high=ll[1], test_weight_low=ll[2], defective_kernels=ll[3], insect_damaged_kernels=ll[4], spotted_kernels=ll[5], broken_kernels=ll[
                                                 6], sprouted_kernels=ll[7], moldy_kernels=ll[8], heat_damaged_kernels=ll[9], nature_heat_damaged_kernels=ll[10], drying_heat_damaged_kernels=ll[11], foreign_matter=ll[12], throughs=ll[13], inorganic_impurities=ll[14], organic_impurities=ll[15], colour=ll[16], odour=ll[17], moisture_content=ll[18], severely_moldy_kernels=ll[19])
        conn.save()
...
def packagindefault():
    ...
    str_sql = 'truncate table `grain_in_packaging`; '
    cursor = connection.cursor()
    cursor.execute(str_sql)
    cursor.close()
    connection.close()
    for ll in DEFAULT:
        a = pypinyin.pinyin(ll[0], style=pypinyin.STYLE_FIRST_LETTER)
        b = []
        for l1 in range(len(a)):
            b.append(str(a[l1][0]).title())
        c = "".join(b)
        defaultins = Packaging.objects.create(
            packag_name=ll[0], find_code=ll[1], py_abbreviation=c)
        defaultins.save()

# 车种默认值


def vehicletypedefault():
    DEFAULT = [
        ['汽车', '1'],
        ['四轮车', '2'],
        ['农用车', '3'],
    ]
    str_sql = 'truncate table `grain_in_vehicletype`; '
    cursor = connection.cursor()
    cursor.execute(str_sql)
    cursor.close()
    connection.close()
    for ll in DEFAULT:
        ...
        for l1 in range(len(a)):
            b.append(str(a[l1][0]).title())
        c = "".join(b)
        defaultins = VehicleType.objects.create(
            vehicle_type=ll[0], find_code=ll[1], py_abbreviation=c)
        defaultins.save()
