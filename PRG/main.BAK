...
SET NOTIFY OFF
SET TALK OFF
PUBLIC MAINPATH,SERVERPATH,DLLPATH,REPORTPATH,PROJECTPATH,TOOLSPATH,LOGSPATH,LOCALUUID,SURL,RETUUID,DEVUUID
PUBLIC ENTER_NAME,ENTER_UNCODE,ENTER_START,ENTER_END,ENTER_FREE,VERSION_NUM,APP_NAME,ENTER_CODE
...
IF !FILE('c:\windows\xyms.xy') THEN
	INIWRITE('dev_code',GETGUID(3),'DevInformations','c:\windows\xyms.xy')
	DEVUUID=INIREAD('dev_code','','DevInformations','c:\windows\xyms.xy')
ELSE
	DEVUUID=INIREAD('dev_code','','DevInformations','c:\windows\xyms.xy')
ENDIF
LOCALUUID=GETGUID(3)
...
IF VRET=='ok' THEN
	...
	IF JSON_CHILDS(RETU_VRET)>0 THEN
		FOR X=1 TO JSON_CHILDS(RETU_VRET) STEP 1
			...
			STORE JSON_VALUE(JSON_VALUE(RETU_VRET,X),'enter_code') TO ENTER_CODE
