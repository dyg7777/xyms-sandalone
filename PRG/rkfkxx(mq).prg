...
SCAN
	...
	EOLE.CELLS(5+I,2).VALUE=ALLTRIM(PRINTLIST.化验单号)
	EOLE.CELLS(5+I,3).VALUE=ALLTRIM(PRINTLIST.品名)
	EOLE.CELLS(5+I,4).VALUE=ALLTRIM(PRINTLIST.等级)
	EOLE.CELLS(5+I,5).VALUE=PRINTLIST.水份
	EOLE.CELLS(5+I,6).VALUE=PRINTLIST.单价
	EOLE.CELLS(5+I,7).VALUE=PRINTLIST.净重
	EOLE.CELLS(5+I,8).VALUE=PRINTLIST.实付金额
	EOLE.CELLS(5+I,9).VALUE=ALLTRIM(PRINTLIST.姓名)
	EOLE.CELLS(5+I,10).VALUE=ALLTRIM(PRINTLIST.SLRXB)
	EOLE.CELLS(5+I,11).VALUE=ALLTRIM(PRINTLIST.身份证号)
*	EOLE.CELLS(5+I,12).VALUE=PRINTLIST.净重-PRINTLIST.纯重
	EOLE.CELLS(5+I,13).VALUE=ALLTRIM(PRINTLIST.乡村)
*	EOLE.CELLS(5+I,14).VALUE=
	EOLE.CELLS(5+I,15).VALUE=ALLTRIM(PRINTLIST.售粮人开户)
	I=I+1
ENDSCAN
STORE ALLTRIM(PRINTLIST.品名) TO pm
EOLE.CELLS(1,1).VALUE=ALLTRIM(DW_1)+pm+'粮食日结算明细表'
IF DATE1=DATE2 THEN
	EOLE.CELLS(2,1).VALUE=ALLTRIM(tTOC(DATE1))
ELSE
	EOLE.CELLS(2,1).VALUE=ALLTRIM(tTOC(DATE1))+'至'+ALLTRIM(tTOC(DATE2))
ENDIF
EOLE.CELLS(3,1).VALUE='存货人：'+hqrmc
EOLE.CELLS(5,1).VALUE='合计'
...
EOLE.CELLS(REC+6,1).VALUE='      ：'+SPACE(28)+'        ：'+SPACE(28)+'驻库员：'+SPACE(28)+'监管员：'
